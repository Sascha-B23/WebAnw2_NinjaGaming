...
class BestellungDao {
    ...
    loadById(id) {
        ...
        for (i = 0; i < result.bestellpositionen.length; i++) {
            result.total.netto += result.bestellpositionen[i].nettopreis;
            result.total.brutto += result.bestellpositionen[i].bruttopreis;
            result.total.mehrwertsteuer += result.bestellpositionen[i].mehrwertsteuer;
        }
        ...
        for (var i = 0; i < result.length; i++) {
            ...
            for (var element of positions) {
                if (element.bestellung.id == result[i].id) {
                    result[i].total.netto += element.nettopreis;
                    result[i].total.brutto += element.bruttopreis;
                    result[i].total.mehrwertsteuer += element.mehrwertsteuer;
    ...
    update(id, bestellzeitpunkt = null, bestellerid = null, zahlungsartid = null, bestellpositionen = []) {
        ...
        if (bestellpositionen.length > 0) {
            for (var element of bestellpositionen) {
                bestellpositionDao.create(id, element.spiel.id, element.gekauft, element.leihzeit);
            }
        }
